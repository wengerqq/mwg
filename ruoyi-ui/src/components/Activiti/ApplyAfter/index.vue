<template>
  <div>
    ...
    <inspection-form
      v-if="row.processKey === 'qualitylnspection'"
      :row="row"
      :taskId="taskId"
      :taskName="taskName"
      :type="type"
...
<script>
  ...
  import InspectionForm from "@/views/purchasee/inspection/inspectionForm";


  export default {
    name: "ApplyAfter",
    components: {DemoForm, LeaveForm,InspectionForm},
    props: {
      /* 表格行数据 */
      row: {
        type: Object,
    ...
    methods: {
      _handleView: function () {
        switch (this.row.processKey) {
          ...
          case 'qualitylnspection':
            this.$refs.inspectionForm.form = this.row;
            this.$refs.inspectionForm.open = true;
            break;
